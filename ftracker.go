...
func RunningSpentCalories(action int, weight, duration float64) float64 {
	...
	var (
		...
		speedKmH float64 = meanSpeed(action, duration)
		//Калории потраченные при беге
		ccalRun float64 = (runningCaloriesMeanSpeedMultiplier * speedKmH * runningCaloriesMeanSpeedShift) * weight / mInKm * duration * minInH
	)
	return ccalRun
}
...
//
// action int — количество совершенных действий(число шагов при ходьбе и беге, либо гребков при плавании).
// duration float64 — длительность тренировки в час
// weight float64 — вес пользователя.
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	...
	var (
		...
		speedMetS float64 = meanSpeed(action, duration) * kmhInMsec
		//Рост пользователя в метрах
		heightMet = height / cmInM
		//Калории потраченные при ходьбе
		ccalWalk = (walkingCaloriesWeightMultiplier*weight + (speedMetS*speedMetS/heightMet)*walkingSpeedHeightMultiplier*weight) * duration * minInH
	)
	return ccalWalk
}
...
func SwimmingSpentCalories(lengthPool, countPool int, duration, weight float64) float64 {
	...
	var (
		//Средняя скорость в км/ч
		speedSwim = swimmingMeanSpeed(lengthPool, countPool, duration)
		//Калории потраченные при плавании
		ccalSwim = (speedSwim + swimmingCaloriesMeanSpeedShift) * swimmingCaloriesWeightMultiplier * weight * duration
	)
	return ccalSwim
}
